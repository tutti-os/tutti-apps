...
- Cause: the date picker did not pass the app locale into `react-day-picker`, so the calendar used the default English month/year order and rendered the month dropdown before the year dropdown in Chinese UI.
- Fix: map the app locale to DayPicker's date-fns locale and pass it into the shared calendar so Chinese captions render year-first while English keeps the default order.
- Verification:
  ...
  - `pnpm exec biome check apps/daily-tech-radar/src/components/app-shell.tsx apps/daily-tech-radar/src/components/app-shell.test.tsx` passed.
